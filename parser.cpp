...
int Parser::check(const std::string path)
{
	...
	while (f.eof() == false)
	{
		...
		Lexeme inputw(atoi(tmp.substr(0, pos - 0).c_str()), tmp.substr(pos3+1,pos2-pos3-1), tmp.substr(pos2 + 1, tmp.size() - pos2 - 1), nodeID);
...
void Parser::read_grammer(const std::string path)
{
	...
	if (f.is_open())
	{
		...
		while (f.eof() == false)
		{
			...
			InfoSymbol new_symbol(tmp.substr(0, pos - 0), NON_TERMINATOR, generators_index++);
			num_nontermi++;
	...
	if (f.is_open())
	{
		...
		// besides, in this part we link every non_terminator to generators whose left InfoSymbol is current non_terminator 
		//the contradiction is that we want to get the generators by InfoSymbol itself, but the generators are changing,we solve it by overloading < between symbols

		ProductionRule new_generator;
		std::string tmp;
		unsigned int start_pos = 0;
		unsigned int pos;
		unsigned int order = 0;  // the order the current ProductionRule is .
		while (f.eof() == false)
		{
			...
			InfoSymbol new_nonterninator(tmp.substr(0, pos - 0), 1);
			...
			while (pos + 1 < tmp.size())
			{
				...
				InfoSymbol new_symbol(tmp.substr(start_pos, pos - start_pos), NON_TERMINATOR);
			...
			of << '\n';
			//here we get a new ProductionRule, push it into generators_list;
			...
			if (ite != non_terminators.end())
			{
				...
				{
					std::vector<ProductionRule> new_list;
...
void Parser::get_symbol_first(const InfoSymbol& a)
{
	//if (a.name == "operator")
		//std::cout << "g";
	...
	for (auto const& i : generators_list.at(index))
	{
		...
		for (auto sym : i.right_list)
		{
			...
			if (tmp->second.find(InfoSymbol("$")) == tmp->second.end()) // the episilon is not in the first set of sym
			{
				have_epsilon = false;
			...
			{
				std::set<InfoSymbol> temp = tmp->second;
				temp.erase(InfoSymbol("$", TERMINATOR, -1));  //ERASE THE EPISILON when merging the first set
		...
		{
			first_set.insert(InfoSymbol("$", TERMINATOR, -1));
		}
	}
	symbol2first.insert(std::make_pair(a.name, first_set));
	return;
...
void Parser::get_all_symbol_first()
{
	...
	{
		of << "FIRST['" << i.first << "'] = { ";
		for (std::set<InfoSymbol>::iterator ite = i.second.begin(); ite != i.second.end(); ite++)
		{
			of << "'" << ite->name << "'" << " ";
		}
		of << "}\n";
...
void Parser::get_closure(ItemSet& group)
{
...
void Parser::get_sequence_first(const std::vector<std::string>& seq, std::vector<std::string>& re)
{
	...
	{
		std::set<InfoSymbol> tmp = this->symbol2first.find(i)->second;

		//no epsilon
		if (tmp.find(InfoSymbol("$", TERMINATOR, -1)) == tmp.end())
		{
...
void Parser::get_item_group_list()
{
	...
	std::set<InfoSymbol> tmp;
	tmp.insert(InfoSymbol("#", TERMINATOR, -1));
	symbol2first.insert(std::make_pair("#", tmp));

	//first item group
	ItemSet first_group(0);
	InfoSymbol first_symbol("sstart", NON_TERMINATOR);
...
void Parser::item_group_go(const ItemSet& scr, ItemSet& dst, std::string input)
{
	...
	for (const auto& i : scr.items)
	{
		...
		temp2.clear();
		s.clear();
		if (i.index < i.base.right_list.size() && input == i.base.right_list.at(i.index).name) //at least one InfoSymbol behind the point 
		{
...
void Parser::get_LR1_table()
{
	...
	for (const auto& item_group_unit : item_groups)
	{
		std::set<LR1VerificationEntry> s;
		...
		for (const auto& item_unit : item_group_unit.items)
		{
			if (item_unit.index < item_unit.base.right_list.size())
			{
				...
				InfoSymbol a_symbol = item_unit.base.right_list.at(item_unit.index);
				...
				if (a_symbol.type == TERMINATOR)
				{
					...
					if (ite != go_map.at(index).end()) //we get it
					{
						...
						auto  it = s.find(LR1VerificationEntry(index, a_symbol.name, dst));
						if (it == s.end())
						{
							s.insert(LR1VerificationEntry(index, a_symbol.name, dst));

							Action* new_action = new Action(a_symbol.name, dst);
							//link this new unit to the index-th position of LR1 map

							//insert into the head!
							Action* tmp = LR1_table.at(index).action_ptr->next;
							new_action->next = tmp;
				...
				if (item_unit.prospect_symbols.size() == 1)
				{
					for (const auto& i : item_unit.prospect_symbols)
					{
						if (i == "#")
						{
							...
							auto  it = s.find(LR1VerificationEntry(index, "#", ACC));
							if (it == s.end())
							{
								s.insert(LR1VerificationEntry(index, "#", ACC));

								Action* new_action = new Action("#", ACC);
								//link this new unit to the index-th position of LR1 map
				...
				if (item_unit.index == item_unit.base.right_list.size())
				{
					if (item_unit.base.right_list.at(0).name != "$")
					{
						for (const auto& symbol_unit : item_unit.prospect_symbols)
						{
							...
							auto  it = s.find(LR1VerificationEntry(index, symbol_unit, item_unit.base.order));
							if (it == s.end())
							{
								s.insert(LR1VerificationEntry(index, symbol_unit, item_unit.base.order));

								Action* new_action = new Action(symbol_unit, item_unit.base.order);
								//link this new unit to the index-th position of LR1 map
		...
		for (const auto& i : non_terminators)
		{
			...
			if (ite != go_map.at(index).end())
			{
				...
				auto  it = s.find(LR1VerificationEntry(index, i.name, dst));
				if (it == s.end())
				{
					s.insert(LR1VerificationEntry(index, i.name, dst));

					StateTransition* new_goto = new StateTransition(i.name, dst);
					//link this new unit to the index-th position of LR1 map

					//insert into the head!
					StateTransition* tmp = LR1_table.at(index).goto_ptr->next;
					new_goto->next = tmp;
...
void Parser::print_LR1_table()
{
	...
	for (unsigned int i = 0; i < LR1_table.size(); i++)
	{
		...
		of << std::setw(5) << i << "\n";
		of << "ACTION\n";
		Action* track = LR1_table.at(i).action_ptr->next;
		while (track != nullptr)
		{
		...
		StateTransition* track2 = LR1_table.at(i).goto_ptr->next;
